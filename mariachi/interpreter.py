...
class Interpreter:
    ...
    def visit_NumberNode(self, node, context):
        return RTResult().success(
            Number(node.tok.value).with_meta(context, node.pos_start, node.pos_end)
        )
        ...
        if not value:
            return res.failure(
                EjecucionError(
                    node.pos_start,
                    node.pos_end,
                    f"'{var_name}' no es definido",
                    context,
                )
            )
        value = (
            value.copy().set_position(node.pos_start, node.pos_end).set_context(context)
        )
        return res.success(value)
    ...
    def visit_ConstAssignNode(self, node, context):
        ...
        try:
            ...
            return res.failure(
                EjecucionError(node.pos_start, node.pos_end, str(e), context)
            )
...
class SymbolTable:
    ...
    def set_const(self, name, value):
        if name in self.symbols or name in self.constants:
            raise Exception(
                f"'{name}' ya está definido y no se puede redefinir como constante"
            )
...
class Value:
    ...
    def illegal_operation(self, other=None):
        ...
        return EjecucionError(
            self.pos_start, other.pos_end, "Operación no permitida", self.context
        )
        ...
        if isinstance(other, Number):
            ...
            try:
                ...
                return None, EjecucionError(
                    other.pos_start,
                    other.pos_end,
                    "El indice esta afuera de la lista",
                    self.context,
                )
        else:
            return None, self.illegal_operation(self, other)
